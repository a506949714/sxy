...
void ClientServer::translateData(QTcpSocket *socket, const QByteArray &data)
{
    ...
    else if(type=="ctrldata")
            {
                //该数据(QByteArray)是控制数据
                //将数据转发给网关服务类(GatewayServer)
                QString boxID=json.value("boxID").toString();
                //qDebug()<<"bb";
                emit sendGateWayServer(boxID,data);
            }
    else if(type=="historydata")
    {
        QString tableName=json.value("name").toString();
        QString hour=json.value("hour").toString();
        QJsonObject json=db->selectData(tableName,hour.toInt());
        if(tableName=="Temp")
        {
            QByteArray data=ClientPotocol::packHistoryData("historyTemp",json);
            socket->write(data);
        }
...
//        qDebug()<<"电磁锁："<<boxHash.value(boxID)->lock;
    db->insertTemp(boxHash.value(boxID)->temp1);
}
