...
GatewayServer::GatewayServer(QObject *parent) : QTcpServer(parent)
{
    ...
    connect(ClientServer::getClientServer(),SIGNAL(sendGateWayServer(QString,QByteArray)),this,SLOT(receiveClientServerSlot(QString,QByteArray)));
}
...
//接收客户端服务类转发来的控制数据
void GatewayServer::receiveClientServerSlot(const QString &boxID, const QByteArray &data)
{
    QHash<GatewaySocket*,QThread*>::const_iterator it;
    for(it=hash.begin();it!=hash.end();it++)
    {
        if(it.key()->getBoxID()==boxID)
        {
            it.key()->write(data);
            break;
        }
